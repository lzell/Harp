 ...
 */
    ...
    func didReceiveRequestForController(name: String, from host: Host) {
        ...
        if let nextClass = NSClassFromString(bundleName + "." + name) as? RemoteViewController.Type {
            let next = nextClass.init(clientUDPAddress: receiveAddr!)
            ...
            assert(false, "Bad controller name!")
        }
    }
